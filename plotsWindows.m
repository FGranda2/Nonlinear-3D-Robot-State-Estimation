...
newSize = size(T_FINAL);
r_GT = r_i_vk_i(1:3,kinit:kend);
theta_GT = theta_vk_i(1:3,kinit:kend);

for i = 1:newSize(1,1)
    
    Cov_matrix = inv(A{i,1});
    ...
    x_vals(i,2) = 3*sqrt(Var_values(1));
    y_vals(i,2) = 3*sqrt(Var_values(2));
    z_vals(i,2) = 3*sqrt(Var_values(3));
    ...
    xth_vals(i,2) = 3*sqrt(Var_values(4));
    yth_vals(i,2) = 3*sqrt(Var_values(5));
    zth_vals(i,2) = 3*sqrt(Var_values(6));
