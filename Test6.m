...
for ite = 1:20
    ...
    for i = k1:k2
       if i == k1
           ...
           T_div = T_0 / T_op{store,1};
           r =inv(T_div) * [0;0;0;1];
           r = DT*r;
           C = T_div(1:3,1:3);
           psi = rot2vec(C);
           e_vk(1:6,store) = [r;psi];
           ...
           F_kpre{store-1,1} = tranAd(T_op{store,1} / T_op{store-1,1});
       end
       % Measurement model
       ...
       if num(store) > 0
           [y_jk, g_k, G_cam, P_j ] = measModel2(av_points, av_pos, DT, T_cv, T_op{store,1}, fu, fv, cu, cv, b);
           ...
           for j = 1:num(store)
               ...
               G_instk(j*4 - 3:j*4, :) = G_cam{j,1} * DT * T_cv * point2fs(T_op{store,1} * P_j(1:4,j));
           ...
           %e_jk{store,1} = e_jk{store-1,1};
           %G_jk{store,1} = G_jk{store-1,1};
           %R_F{store,1} = R_F{store-1,1};
           e_jk{store,1} = zeros(4,1);
           G_jk{store,1} = zeros(4,6);
           R_F{store,1} = R;
       end    
       store=store+1;
    end
